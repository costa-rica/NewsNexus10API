...
function ensureLogDirectory() {
  if (isProduction && !isChildProcess) {
    try {
      ...
      console.error(
        `[${appName}] Failed to create log directory: ${err.message}`
      );
...
const productionFormat = winston.format.combine(
  ...
  winston.format.printf(({ timestamp, level, message, stack, ...meta }) => {
    ...
    return `[${timestamp}] [${level
      .toUpperCase()
      .padEnd(5)}] [${processId}] ${message}${metaStr}${stackStr}`;
  })
);
...
const developmentFormat = winston.format.combine(
  ...
  winston.format.printf(({ timestamp, level, message, stack, ...meta }) => {
    ...
    return `${timestamp} ${level.padEnd(
      15
    )} [${processId}] ${message}${metaStr}${stackStr}`;
  })
);
  ...
  if (isProduction) {
    ...
    if (canWriteFiles) {
      try {
        ...
        console.error(
          `[${appName}] Failed to initialize file logging: ${err.message}`
        );
...
const logDestination =
  isProduction && !isChildProcess ? `files in ${logDir}` : "console";

logger.info(
  `Winston logger initialized: ${mode} mode, ${processType}, logging to ${logDestination}`
);
